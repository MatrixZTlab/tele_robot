...
class WebRTC_PublisherThread(threading.Thread):
    ...
    async def _offer(self, request: web.Request) -> web.Response:
        ...
        # 解析并打印客户端 SDP 中的 ICE candidate（如果有）
        # client_sdp = params.get("sdp", "") or ""
        # client_candidates = []
        # for line in client_sdp.splitlines():
        #     if line.startswith("a=candidate:") or line.startswith("candidate:"):
        #         parts = line.split()
        #         addr = parts[4] if len(parts) >= 6 else None
        #         cand_type = None
        #         if "typ" in parts:
        #             try:
        #                 cand_type = parts[parts.index("typ") + 1]
        #             except Exception:
        #                 cand_type = None
        #         client_candidates.append({"raw": line, "addr": addr, "type": cand_type})

        # if client_candidates:
        #     logger_mp.info("[_offer] 客户端 ICE 候选 (%d): %s", len(client_candidates), client_candidates)
        # else:
        #     logger_mp.info("[_offer] 未在 SDP 中发现 ICE 候选（可能使用 trickle-ice）")

        pc = RTCPeerConnection(
            configuration=RTCConfiguration(
                iceServers=[
                    RTCIceServer(urls="stun:stun.l.google.com:19302"),          # STUN 仍然保留
                    # RTCIceServer(urls="turn:192.168.1.10:3478",                # TURN 服务器
                    #             username="webrtc",
