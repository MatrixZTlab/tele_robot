...
logging_mp.getLogger("aiortc").setLevel(logging_mp.DEBUG)
logging_mp.getLogger("aioice").setLevel(logging_mp.DEBUG)
import os
import argparse
import glob
import cv2
...
import subprocess
import platform
from teleimager.image_client import TripleRingBuffer, ZMQ_PublisherManager, ZMQ_Responser
# webrtc dependencies
import asyncio
import json
            ...
            if (pc.iceGatheringState === 'complete') {
                ...
                const checkState = () => {
                    if (!pc || pc.iceGatheringState === 'complete') {
                        if (pc) pc.removeEventListener('icegatheringstatechange', checkState);
                        resolve();
                    }
                };
...
class WebRTC_PublisherThread(threading.Thread):
    ...
    async def _offer(self, request: web.Request) -> web.Response:
        ...
        logger_mp.info("[DEBUG] Registering icecandidate and icegatheringstatechange listeners")
        candidates = []
        ice_gathering_complete = asyncio.Event()

        @pc.on("icecandidate")
        def on_icecandidate(candidate):
            logger_mp.info(f"[DEBUG] icecandidate fired, candidate is None: {candidate is None}")
            if candidate:
                sdp_line = "a=" + candidate_to_sdp(candidate)
                logger_mp.info(f"[DEBUG] collected candidate: {sdp_line}")
                candidates.append(sdp_line)
            else:
                logger_mp.info("[DEBUG] icecandidate with null candidate (end of candidates)")

        @pc.on("icegatheringstatechange")
        def on_icegatheringstatechange():
            logger_mp.info(f"[DEBUG] ICE gathering state changed to: {pc.iceGatheringState}")
            if pc.iceGatheringState == "complete":
                logger_mp.info("[DEBUG] ICE gathering complete, setting event")
                ice_gathering_complete.set()

        await pc.setRemoteDescription(offer)
        answer = await pc.createAnswer()
        await pc.setLocalDescription(answer)
        logger_mp.info(f"[DEBUG] After setLocalDescription, ICE gathering state: {pc.iceGatheringState}")

        # 等待 gathering 完成
        if pc.iceGatheringState != "complete":
            logger_mp.info("[DEBUG] Waiting for ICE gathering to complete...")
            try:
                await asyncio.wait_for(ice_gathering_complete.wait(), timeout=5.0)
                logger_mp.info("[DEBUG] ICE gathering completed within timeout")
            except asyncio.TimeoutError:
                logger_mp.warning("[WebRTC] ICE gathering timed out, proceeding with current candidates")

        if not candidates:
            logger_mp.info("[DEBUG] Checking local candidates directly from transports...")
            try:
                if hasattr(pc, "_RTCPeerConnection__transports"):
                    transports = pc._RTCPeerConnection__transports
                    for transport in transports:
                        ice = None
                        if hasattr(transport, "_RTCIceTransport__ice"):
                            ice = transport._RTCIceTransport__ice
                        elif hasattr(transport, "iceGatherer"):
                            ice = transport.iceGatherer

                        if ice is None:
                            logger_mp.warning("[DEBUG] Transport has no accessible ICE object")
                            continue

                        logger_mp.info(f"[DEBUG] ICE object type: {type(ice).__name__}")
                        for attr_name in ("local_candidates", "_local_candidates", "getLocalCandidates"):
                            if hasattr(ice, attr_name):
                                attr = getattr(ice, attr_name)
                                local_candidates = attr() if callable(attr) else attr
                                for candidate in local_candidates:
                                    logger_mp.info(f"[DEBUG] Local candidate found: {candidate}")
                                break
                        else:
                            logger_mp.warning("[DEBUG] Cannot find local candidates attribute on ICE object")
                else:
                    logger_mp.warning("[DEBUG] Cannot access internal transports, skipping")
            except Exception as e:
                logger_mp.error(f"[DEBUG] Error while checking local candidates: {e}")

        logger_mp.info(f"[DEBUG] Candidates collected: {len(candidates)}")
        for idx, c in enumerate(candidates):
            logger_mp.info(f"[DEBUG] Candidate {idx}: {c}")

        final_sdp = pc.localDescription.sdp.rstrip("\r\n")
        if candidates:
            final_sdp += "\r\n" + "\r\n".join(candidates)
        if pc.iceGatheringState == "complete":
            final_sdp += "\r\na=end-of-candidates"
        ...
        logger_mp.info(f"[DEBUG] Final SDP length: {len(final_sdp)}")

        return web.Response(
            content_type="application/json",
            text=json.dumps({"sdp": final_sdp, "type": pc.localDescription.type}),
            headers={
                "Access-Control-Allow-Origin": "*",
...
class CameraFinder:
    ...
    """
    def __init__(self, realsense_enable=False, orbbec_enable=False, verbose=False):
        ...
        if realsense_enable:
            ...
            self.ob_serial_numbers = []
            self.ob_video_paths = []
            self.ob_rgb_video_paths = []
        elif orbbec_enable:
            self.ob_serial_numbers = self._list_orbbec_serial_numbers()
            self.ob_video_paths = self._list_orbbec_video_paths()
            self.ob_rgb_video_paths = [p for p in self.ob_video_paths if self._is_like_rgb(p)]
    ...
    def _list_uvc_rgb_video_paths(self):
        return [p for p in self.video_paths if self._is_like_rgb(p) and p not in self.rs_video_paths and p not in self.ob_video_paths]
    ...
    def _list_orbbec_video_paths(self):
        def _read_text(path):
            try:
                with open(path, "r", encoding="utf-8", errors="ignore") as f:
                    return f.read().strip()
        ...
        for devnode in sorted(glob.glob("/dev/video*")):
            ...
            # Match Orbbec by name and Orbbec vendor ID
            if name and "orbbec" in name.lower() and (vendor_id or "").lower() in ("2bc5",):
                ports.append(devnode)
    ...
    def _list_orbbec_serial_numbers(self):
        try:
            import pyorbbecsdk as ob
        except ImportError:
            raise RuntimeError(
                "[Orbbec] pyorbbec not installed. Please install it according to your platform:\n"
        ...
        for i in range(device_list.get_count()):
            ...
            try:
                serials.append(device.get_device_info().get_serial_number())
            except Exception:
                continue
        return serials
    ...
    def is_ob_serial_exist(self, serial_number):
        return str(serial_number) in self.ob_serial_numbers
    ...
    def info(self):
        ...
        if self.ob_serial_numbers:
            logger_mp.info("----------------------- Orbbec Cameras ----------------------------------")
            logger_mp.info(f"Orbbec serial numbers: {self.ob_serial_numbers}")
            logger_mp.info(f"Orbbec video paths: {self.ob_video_paths}")
            logger_mp.info(f"Orbbec RGB-like video paths: {self.ob_rgb_video_paths}")
...
class OrbbecCamera(BaseCamera):
    def __init__(self, cam_topic, serial_number, img_shape, fps, 
        ...
        try:
            import pyorbbecsdk as ob
            from teleimager.utils import frame_to_bgr_image
        except Exception as e:
            raise RuntimeError(
                f"[Orbbec] pyorbbec or utils.frame_to_bgr_image import failed: {e}. Please ensure pyorbbec is installed according to your platform:\n"
                "    pip install pyorbbec\n"
            )
        ...
        try:
            ctx = ob.Context()
            devices = ctx.query_devices()
            target_device = devices.get_device_by_serial_number(str(self._serial_number))
            self._device = target_device

            self.pipeline = ob.Pipeline(target_device)
            config = ob.Config()
            color_profile_list = self.pipeline.get_stream_profile_list(ob.OBSensorType.COLOR_SENSOR)
            color_profile:ob.VideoStreamProfile = color_profile_list.get_video_stream_profile(self._img_shape[1], self._img_shape[0], ob.OBFormat.RGB, self._fps)
            config.enable_stream(color_profile)
            if self._enable_depth:
                depth_profile_list = self.pipeline.get_stream_profile_list(ob.OBSensorType.DEPTH_SENSOR)
                depth_profile = depth_profile_list.get_video_stream_profile(self._img_shape[1], self._img_shape[0], ob.OBFormat.Y16, self._fps)
            ...
            if self.pipeline:
                try:
                    ...
                    pass
            raise RuntimeError(f"[OrbbecCamera] Failed to initialize Orbbec camera {self._serial_number}: {e}")
        
    def __str__(self):
        return (
            f"[OrbbecCamera: {self._cam_topic}] initialized with "
    ...
    def _update_frame(self):
        ...
        bgr_numpy = self.frame_to_bgr_image(color_frame)
        if self._enable_webrtc:
        ...
        try:
            if hasattr(self.pipeline, "stop") and getattr(self.pipeline, "_running", False):
                try:
                    ...
                    logger_mp.warning(f"[OrbbecCamera] pipeline.stop() failed: {e}")
        except Exception:
            pass
        self.pipeline = None
        logger_mp.info(f"[OrbbecCamera] Released {self._cam_topic}")
...
class IsaacSimCamera(BaseCamera):
    def __init__(self, cam_topic, img_shape, fps,
        ...
        """
        super().__init__(cam_topic, img_shape, fps, enable_zmq, zmq_port, enable_webrtc, webrtc_port, webrtc_codec)
        from tools.shared_memory_utils import MultiImageReader # Shared memory utility for sim
...
# ========================================================
class ImageServer:
    def __init__(self, cam_config, realsense_enable=False, orbbec_enable=False, camera_finder_verbose=False, isaacsim_enable=False):
        self._cam_config = cam_config
        self._realsense_enable = realsense_enable
        self._orbbec_enable = orbbec_enable
        self._isaacsim_enable = isaacsim_enable
        self._stop_event = threading.Event()
        self._cameras: dict[str, BaseCamera] = {}
        if not self._isaacsim_enable:
            self._cam_finder = CameraFinder(realsense_enable, orbbec_enable, camera_finder_verbose)
            ...
            for cam_topic, cam_cfg in self._cam_config.items():
                ...
                if cam_type == "opencv":
                    ...
                    if not self._orbbec_enable:
                        self._cameras[cam_topic] = None
                        logger_mp.error(f"[Image Server] Please start image server with the '--ob' flag to support Orbbec {cam_topic}.")
                    elif not self._cam_finder.is_ob_serial_exist(serial_number):
                        self._cameras[cam_topic] = None
        ...
        try:
            ...
            while not self._stop_event.is_set():
                try:
                    ...
                    print(f"[Image Server] Error updating frame for {cam_topic} camera: {e}")
...
def run_isaacsim_server():
    ...
    try:
        ...
        exit(1)
    # start image server
    server = ImageServer(cam_config, realsense_enable=False, orbbec_enable=False, camera_finder_verbose=False, isaacsim_enable=True)
    server.start()
    return server
...
def main():
    ...
    parser.add_argument('--ob', action = 'store_true', help = 'Enable Orbbec camera mode. Otherwise only find UVC/OpenCV cameras.')
    ...
    if args.cf:
        CameraFinder(realsense_enable=args.rs, orbbec_enable=args.ob, verbose=True)
        exit(0)
    ...
    # start image server
    server = ImageServer(cam_config, realsense_enable=args.rs, orbbec_enable=args.ob, camera_finder_verbose=False)
    server.start()
