...
目前 Tele Imager 用于 xr_teleoperate 项目中提供遥操作视频流。
...
user@ubuntu:~$ mkdir -p ~/miniconda3
user@ubuntu:~$ wget https://repo.anaconda.com/miniconda/Miniconda3-latest-Linux-aarch64.sh -O ~/miniconda3/miniconda.sh
user@ubuntu:~$ bash ~/miniconda3/miniconda.sh -b -u -p ~/miniconda3
user@ubuntu:~$ rm ~/miniconda3/miniconda.sh
user@ubuntu:~$ source ~/miniconda3/bin/activate
(base) user@ubuntu:~$ conda init --all
```

2. 创建并激活 conda 环境：

```
(base) user@ubuntu:~$ conda create -n teleimager python=3.10 -y
(base) user@ubuntu:~$ conda activate teleimager
```

3. 安装项目与依赖：

```
(teleimager) user@ubuntu:~$ sudo apt install -y libusb-1.0-0-dev libturbojpeg-dev
(teleimager) user@ubuntu:~$ git clone http://10.110.150.241:5656/zt_ai/robot_control/tele_robot
(teleimager) user@ubuntu:~$ cd teleimager
# 假如您只使用客户端
(teleimager) user@ubuntu:~/teleimager$ pip install -e .
# 假如您还使用服务端
(teleimager) user@ubuntu:~/teleimager$ pip install -e ".[server]"
```
...
5. 配置证书路径（WebRTC 模式需要）
    证书通常由 [televuer](<televuer-link>) 仓库生成。
...
你将看到类似下面的输出：
 ```bash
 (teleimager) user@ubuntu:~$ python -m teleimager.image_server --cf
